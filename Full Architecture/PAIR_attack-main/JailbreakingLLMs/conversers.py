...
from language_models import APILiteLLM, VertexAIDirectModel
from config import FASTCHAT_TEMPLATE_NAMES, GEMINI_MODEL_NAMES, Model
...
def load_indiv_model(model_name, local = False, use_jailbreakbench=True):
    if use_jailbreakbench: 
        ...
        if local:
            ...
            # Route Gemini models to direct Vertex AI REST client
            if Model(model_name) in GEMINI_MODEL_NAMES:
                lm = VertexAIDirectModel(model_name)
            else:
...
class AttackLM():
    ...
    def _generate_attack(self, openai_conv_list: list[list[dict]], init_message: str):
        ...
        if any([output is None for output in valid_outputs]):
            print("debug outputs_list", outputs_list)
