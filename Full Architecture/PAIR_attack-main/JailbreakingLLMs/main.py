import os
os.environ.setdefault("WANDB_MODE", "offline")  # Run WandB offline by default (no login needed)
...
def main(args):
    ...
    transcript_logger = TranscriptLogger(args, system_prompts)
    ...
    for iteration in range(1, args.n_iterations + 1):
        ...
        # Get target responses (with JailVax Middleware Intercept)
        intercepts = [False] * len(adv_prompt_list)
        if verifier:
            for idx, prompt in enumerate(adv_prompt_list):
                partial = {
                    "attack_type": "pair",
                    "goal": args.goal,
                    "streams": [{"iterations": [{"adversarial_prompt": prompt}]}]
                }
                v_res = verifier.verify(partial)
                if v_res.overall_action == Action.BLOCK:
                    print(f"\n  [!] JAILVAX MIDDLEWARE BLOCKED PAIR PROMPT {idx} (Rule: {v_res.triggered_rules[0].rule_id})")
                    intercepts[idx] = True

        to_query = [p for idx, p in enumerate(adv_prompt_list) if not intercepts[idx]]
        query_results = targetLM.get_response(to_query) if to_query else []
        
        target_response_list = []
        q_idx = 0
        for idx in range(len(adv_prompt_list)):
            if intercepts[idx]:
                target_response_list.append("[JAILVAX BLOCKED] I cannot fulfill that request.")
            else:
                target_response_list.append(query_results[q_idx])
                q_idx += 1
        ...
        # Transcript log values
        transcript_logger.log(iteration, extracted_attack_list, target_response_list, judge_scores)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        "gemini-2.0-flash", "gemini-1.5-pro", "gemini-2.5-flash-lite", "gemini-2.5-flash", "mixtral", "vicuna-7b-v1.5", "ollama"]
    )
    parser.add_argument(
        "--attack-max-n-tokens",
        type = int,
    ...
    parser.add_argument(
        ...
        choices=["vicuna-13b-v1.5", "llama-2-7b-chat-hf", "gpt-3.5-turbo-1106", "gpt-4-0125-preview", "claude-instant-1.2", "claude-2.1", "gemini-pro",
        "gemini-2.0-flash", "gemini-1.5-pro", "gemini-2.5-flash-lite", "gemini-2.5-flash", "ollama"]
    )
    parser.add_argument(
        "--target-max-n-tokens",
        type = int,
    ...
    parser.add_argument(
        ...
        type = str,
        default = "transcripts",
        help = "Directory to save JSON transcripts. Default: transcripts/"
    )
    parser.add_argument(
        "--evaluate-locally",
        action = 'store_true',
